...
test_that("sensitivity_plot provides proper messages and warnings",{
  expect_error(sensitivity_plot(SAVF_matrix, weights, names, 4))
  SAVF_matrix<-as.matrix(SAVF_matrix)
  expect_error(sensitivity_plot(SAVF_matrix,weights2,names, 4))
  expect_error(sensitivity_plot(SAVF_matrix,weights=SAVF_matrix,names, 4))
  expect_error(sensitivity_plot(SAVF_matrix[,1:6], weights, names, 4))
  expect_error(sensitivity_plot(SAVF_matrix, weights, c("Greg McElroy","Blaine Gabbert","Christian Ponder"),4))
  expect_warning(sensitivity_plot(SAVF_matrix, c(0.096, 0.224, 0.092, 0.138, 0.150, 0.220, 0.05), names, 4))
  expect_warning(sensitivity_plot(SAVF_matrix, c(0.096, 0.224, 0.092, 0.155, 0.175, 0.228, 0.15), names, 4))
  expect_error(sensitivity_plot(SAVF_matrix, weights, names, "4"))
  expect_error(sensitivity_plot(SAVF_matrix, weights, names, 9))
})